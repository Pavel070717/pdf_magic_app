...
## Corpus Check
- 81 files · ~57,289 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1257 nodes · 2398 edges · 72 communities (55 shown, 11 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 22 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `116f008e`
...
- `reset_database()` --calls--> `init_db()`  [EXTRACTED]
  reset_project.py → utils/database.py
- `reset_database()` --calls--> `init_requisites_db()`  [EXTRACTED]
  reset_project.py → utils/database.py
...
Cohesion: 0.09
Nodes (7): sanitize_text(), TestSanitizeText, Adding material with empty data should not crash., Saving rule with empty from should return 400., Adding object with empty name should return 400., TestHtmlInjection, TestInputValidation
...
Cohesion: 0.11
Nodes (10): clear_state(), route, Tests for routes/dashboard.py — dashboard statistics., TestDashboardStats, patch, TestStartMagic, _atomic_save(), Any (+2 more)
...
### Community 14 - "app.py"
Cohesion: 0.11
Nodes (27): after_request, aocr_page(), converter_page(), directories_page(), health_check(), index(), _kill_port_5000(), log_request_end() (+19 more)
...
### Community 28 - "load_state"
Cohesion: 0.14
Nodes (20): get_registry_data(), get_registry_form(), route, Blueprint: /api/registry/* — registry data CRUD., Return HTML form for the registry modal., save_registry_data(), Tests for utils/state.py — thread-safe atomic state.json persistence., When state.json doesn't exist, load_state returns a default dict. (+12 more)
...
Cohesion: 0.07
Nodes (44): Connection, create_object(), get_aocr_data(), _get_object_name(), list_objects(), route, Blueprint: /api/requisites/* — Управление объектами строительства и их…, GET /api/requisites/<object_id>/aocr — merged data ready for AOCP form… (+36 more)
...
- **Why does `load_state()` connect `load_state` to `save_replace_rules`, `copy_files_worker`, `core.py`, `Flask`, `save_state`, `app.py`, `test_magic_routes.py`, `test_files_routes.py`, `magic.py`, `test_directories_routes.py`, `files.py`?**
  _High betweenness centrality (0.094) - this node is a cross-community bridge._
- **Why does `save_state()` connect `save_state` to `save_replace_rules`, `copy_files_worker`, `core.py`, `Flask`, `app.py`, `conftest.py`, `test_magic_routes.py`, `test_files_routes.py`, `magic.py`, `test_directories_routes.py`, `load_state`, `files.py`?**
  _High betweenness centrality (0.089) - this node is a cross-community bridge._
- **Why does `save_replace_rules()` connect `save_replace_rules` to `save_state`, `test_magic_routes.py`, `copy_files_worker`, `load_state`?**
